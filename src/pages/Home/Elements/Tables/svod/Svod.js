...
import SvodButtons from "./SvodButtons";
import './Svod.css'; 
...
export default function Svod() {
  ...
  const columns = [
    ...
    "Дата поступления", "Бронь", "Период окончания бронирования", "Статус"
  ];
  ...
  useEffect(() => {
    if (editedRows.size > 0) {
      const editRow = async (row) => {
        ...
        try {
          const response = await fetch('/api/editSvod', {
            method: 'POST',
            headers: {
  ...
  const handleExportExcel = () => {
    ...
    XLSX.writeFile(wb, "SvodData.xlsx");
  };

  return (
    <div className='object-table'>
      <SvodButtons onEditToggle={handleEditToggle} onExportExcel={handleExportExcel} isEditing={isEditing} onCopy={handleCopy} />
