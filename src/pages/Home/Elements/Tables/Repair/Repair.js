...
export default function Repair() {
  ...
  const columns = [
    "№","Номер контейнера", "Город", 
    "Терминал","Тип", "Фото", "Состояние", "Оказанный ремонт",
    "Фото после ремонта"
  ];
  ...
  function renderCell(row, col) {
    ...
    if (columnName === "Фото" || columnName === "Фото после ремонта") {
      let filenames;
      try {
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Ремонт");
    XLSX.writeFile(wb, "RepairData.xlsx");
  };
  ...
  return (
    <div className='object-table'>
      ...
      <div className='object-3'>
        ...
        <table className='repair-table' ref={tableRef}>
            <thead>
              <tr>
